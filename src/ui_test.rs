...
macro_rules! test_text_edit {
    ($title:ident {
        ...
        fn $title() {
            ...
            // .skip(1) for first empty line
            let mut editor = Editor::with_lines($before.lines().skip(1), input, Discard, Some((80, 24))).unwrap();
            editor.edit().unwrap();
            let actual = editor.lines().collect::<Vec<_>>();
            // .skip(1) for first empty line
            let expected = $after.lines().skip(1).collect::<Vec<_>>();
            ...
            for (idx, (actual_line, expected_line)) in actual.iter().zip(expected.iter()).enumerate() {
                assert_eq!(expected_line, actual_line, "Line {} mismatch! expected='{:?} actual='{:?}'", idx+1, expected, actual);
            }
        }
    }
}

test_text_edit!(inesrt_char {
    before: "",
...
test_text_edit!(delete_right_char {
    before: "
abc
...
test_text_edit!(delete_until_end_of_line {
    before: "
ab
cd
ef
...
test_text_edit!(delete_until_head_of_line {
    before: "
ab
cd
ef
